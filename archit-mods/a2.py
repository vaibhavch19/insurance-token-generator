...
import json
import logging
from config import FLASK_HOST, FLASK_PORT, REPORT_DIR
...
def raise_ticket(state):
    ...
    # Point to a separate upload form (adjust URL as needed)
    upload_link = f"http://127.0.0.1:5001/upload/{ticket_id}"
    updated_state.update({
        "ticket_id": ticket_id,
...
def generate_report(state):
    ...
    updated_state.update({
        ...
        "messages": [f"Your initial report is ready: {report_link}. Upload additional materials at {state['upload_link']} to update it."],
